...
import { providerConfigurationMockS3 } from '../../helpers/mockCreator';
...
describe('S3Provider', () => {
  ...
  beforeAll(() => {
        ...
        { token: SERVICES.LOGGER, provider: { useValue: jsLogger({ enabled: false }) } },
        {
          ...
          provider: {
            useFactory: (): ProviderManager => {
              return getProviderManager(providerConfigurationMockS3);
            },
          },
        },
      ],
    ...
    s3HelperSource = new S3Helper(providerConfigurationMockS3.source);
    s3HelperDest = new S3Helper(providerConfigurationMockS3.dest);
  });
  ...
  describe('postFile', () => {
    it('When calling postFile, should be able to read the written file from the destination', async () => {
      ...
      const result = await s3HelperDest.readFile(providerConfigurationMockS3.dest.bucket, `${model}/${file}`);
